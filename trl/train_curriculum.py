...
from load_data_curriculum import get_train_data #, val_data
from load_data import val_data # using the same validation data as in the original script
...
# set the mode to offline to avoid syncing with wandb servers
...
def get_model_path(CURRICULUM_STAGE, RANK, LEARNING_RATE, EPOCHS, BETA):
    """
    Returns the model path based on the curriculum stage.
    """
    if CURRICULUM_STAGE == 0:
        return "cjvt/GaMS-9B-Instruct"  # Base model for stage 0
    elif CURRICULUM_STAGE == 1:
        # return f"/ceph/hpc/data/s24o01-42-users/translation_optimization/trl/trained_models/Translation_Curriculum-1-DPO_GaMS-9B_r-{RANK}_lr-{LEARNING_RATE}_b-{BETA}"  # Example path for stage 1
        return "/ceph/hpc/data/s24o01-42-users/translation_optimization/trl/trained_models/Curriculum_DPO_models/GaMS-9B-DPO-Curriculum-0"
        # return "/ceph/hpc/data/s24o01-42-users/translation_optimization/trl/trained_models/Translation_Curriculum-0-DPO_GaMS-9B_r-64_lr-1e-07_b-0.1"
    elif CURRICULUM_STAGE == 2:
        # return f"/ceph/hpc/data/s24o01-42-users/translation_optimization/trl/trained_models/Translation_Curriculum-1-DPO_GaMS-9B_r-{RANK}_lr-{LEARNING_RATE}_b-{BETA}"  # Example path for stage 2
        return "/ceph/hpc/data/s24o01-42-users/translation_optimization/trl/trained_models/Curriculum_DPO_models/GaMS-9B-DPO-Curriculum-1"
    else:
...
def main(train_data, val_data, RANK, LEARNING_RATE, EPOCHS, BETA, CURRICULUM_STAGE):
    ...
    dpo_config = DPOConfig(
        ...
        output_dir=f"training_run/curriculum-{CURRICULUM_STAGE}_r-{RANK}_lr-{LEARNING_RATE}_b-{BETA}",     # Directory where the model predictions and checkpoints will be written
        ...
        run_name=f"Curriculum-{CURRICULUM_STAGE}-DPO_r-{RANK}_lr-{LEARNING_RATE}_e-{EPOCHS}_b-{BETA}",     # Name of the WandB run
    ...
    # Load the model
    model_path = get_model_path(CURRICULUM_STAGE, RANK, LEARNING_RATE, EPOCHS, BETA) # Path to the model based on the curriculum stage
    if local_rank == 0: print("Loading model from:", model_path)
    ...
    if local_rank == 0: 
        print("Saving model")
        model.save_pretrained(f"trained_models/Translation_Curriculum-{CURRICULUM_STAGE}-DPO_GaMS-9B_r-{RANK}_lr-{LEARNING_RATE}_b-{BETA}") # Save the model
...
if __name__ == "__main__":
    ...
    parser.add_argument('--curriculum_stage', default=0, type=int, help='Curriculum stage for curriculum training')
